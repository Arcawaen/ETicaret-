...
namespace Eticaret.WebUI.Areas.Admin.Controllers
{
    ...
    public class ProductImagesController : Controller
    {
        ...
        {
            var databaseContext = _context.ProductImages.Include(p => p.Product).AsQueryable();
            if (productId.HasValue)
            {
                databaseContext = databaseContext.Where(x => x.ProductId == productId.Value);
            }
        ...
        public async Task<IActionResult> Edit(int id, ProductImage productImage, IFormFile? Image, bool cbResmiSil = false)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    if (cbResmiSil) productImage.Image = string.Empty;
                    if (Image is not null)
                    {
                        productImage.Image = await FileHelper.FileLoaderAsync(Image, "/Img/Products/");
                    }
